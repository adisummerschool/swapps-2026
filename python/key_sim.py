...
def get_movement(roll, pitch):
    ...
    if pitch > 0.2:
        mov['front'] = min(1.0, pitch)
    elif pitch < -0.2:
        mov['back'] = min(1.0, -pitch)
    
    if roll > 0.2:
        mov['right'] = min(1.0, roll)
    elif roll < -0.2:
        mov['left'] = min(1.0, -roll)
...
def start_iio(device):
    ...
    dir2key = {'left' : 'a', 'right' : 'd', 'front' : 'w', 'back' : 's'}

    for dir,val in movement.items():
        if val > 0:
            thread = Thread(target=threaded_keypress, args=(dir2key[dir], (TIME * val)))
            thread.daemon = True
            thread.start()
...
def init_iio():
    ctx = iio.Context("ip:10.76.84.20")
