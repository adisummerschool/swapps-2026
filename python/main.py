...
def get_roll_pitch(axis: dict):
    ...
    roll = math.atan2(y, z) * 100 / math.pi
    pitch = math.atan2(-x, math.sqrt(y ** 2 + z ** 2)) * 100 / math.pi
...
def start_iio(device):
    ...
    for dir, value in movement.items():
        if value > 0:
            thread = Thread(target=threaded_keypress, args=(keys[dir], TIME * value))
            thread.daemon = True
            thread.start()
...
def init_iio():
    context = iio.Context("ip:10.76.84.24")
