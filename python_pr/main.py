...
def get_movement(roll, pitch):
    ...
    if roll < 45 and roll > 0:
        mov['right'] = roll / 45
    elif roll > -45 and roll < 0:
        mov['left'] = -roll / 45
    elif roll < -45:
        mov['left'] = 1
        mov['right'] = 0
        ...
        mov['left'] = 0
        mov['right'] = 1
    if pitch < 45 and pitch > 0:
        mov['front'] = pitch / 45
    elif pitch > -45 and pitch < 0:
        mov['back'] = -pitch / 45
    elif pitch < -45:
        mov['back'] = 1
        mov['front'] = 0
...
def start_iio(device):
    ...
    # iterate through each direction and spawn a keypress thread
    #threaded_keypress(key, on_time)
    keyboard_press = {'left':'a', 'right':'d', 'front':'w', 'back':'s'}
    for dir, val in movement.items():
        if val > 0:
            thread = Thread(target=threaded_keypress, args=(keyboard_press[dir], (TIME * val)))
            thread.daemon = True
            thread.start()
    #print(movement)
...
def init_iio():
    ctx = iio.Context("ip:10.76.84.9")
    dev = ctx.find_device("iio_ad5592r_s")
    if dev is None:
...
if __name__ =='__main__':
    ...
    thread.daemon = True
    thread.start()
    with keyboard.Listener(on_press=on_keypress) as listeners:
        listeners.join()
